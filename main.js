...
function sanitizeData(raw) {
  ...
  for (const key of BODY_ORDER) {
    ...
    next.body[key].current = clamp(Number(src.current ?? next.body[key].current) || next.body[key].current, 0, next.body[key].max);
...
function getSelectedToken() { return lastSceneItems.find((item) => item.id === currentSelectionId) ?? null; }
...
function buildMinorDots(token, data) {
  ...
  for (let i = 0; i < data.minor; i += 1) {
    items.push(buildShape().shapeType("CIRCLE").width(8).height(8).position({ x: startX + i * 10, y }).attachedTo(token.id).layer("ATTACHMENT").locked(true).disableHit(true).fillColor("#f59e0b").fillOpacity(0.95).strokeColor("#111827").strokeWidth(1).metadata({ [OVERLAY_KEY]: token.id, kind: "minor", index: i }).build());
  }
  return items;
}
...
function buildSeriousMarks(token, data) {
  ...
  for (let i = 0; i < data.serious; i += 1) {
    items.push(buildShape().shapeType("RECTANGLE").width(4).height(18).position({ x: x - i * 8, y: startY }).attachedTo(token.id).layer("ATTACHMENT").locked(true).disableHit(true).fillColor("#ef4444").fillOpacity(0.95).strokeColor("#111827").strokeWidth(1).cornerRadius(2).metadata({ [OVERLAY_KEY]: token.id, kind: "serious", index: i }).build());
  }
  return items;
}

function buildOverlayLabel(token, data) {
  const cardWidth = Math.max(320, token.width * 2.4);
  const offsetX = token.width / 2 + cardWidth / 2 + 16;
  return buildLabel().name(`Body HP: ${tokenDisplayName(token)}`).plainText(formatOverlayText(data)).fontSize(13).fontWeight(600).lineHeight(1.2).padding(10).textAlign("LEFT").textAlignVertical("MIDDLE").fillColor("#f8fafc").backgroundColor(overlayColor(data)).backgroundOpacity(0.58).strokeColor("#111827").strokeOpacity(0.6).strokeWidth(1).pointerDirection("LEFT").pointerWidth(10).pointerHeight(12).cornerRadius(12).attachedTo(token.id).layer("ATTACHMENT").locked(true).disableHit(true).position({ x: offsetX, y: 0 }).metadata({ [OVERLAY_KEY]: token.id, kind: "card" }).build();
}
...
async function upsertTokenData(tokenId, updater) {
  await OBR.scene.items.updateItems([tokenId], (items) => {
    const token = items[0];
    if (!token) return;
    token.metadata[META_KEY] = sanitizeData(updater(getTokenData(token)));
  });
}
...
async function ensureOverlayForToken(tokenId) {
  ...
  if (ownedOverlays.length) {
    await OBR.scene.items.deleteItems(ownedOverlays.map((i) => i.id));
  }
  if (!data.enabled) return;
  await OBR.scene.items.addItems([buildOverlayLabel(token, data), ...buildMinorDots(token, data), ...buildSeriousMarks(token, data)]);
}
...
async function syncAllOverlays() {
  ...
  try {
    ...
    for (const item of tracked) await ensureOverlayForToken(item.id);
  } catch (err) {
    console.error("Body HP sync failed", err);
  } finally {
    syncing = false;
  }
}
...
async function changeStep(type, delta) {
  const token = getSelectedToken();
  if (!token || playerRole !== "GM") return;
  await upsertTokenData(token.id, (current) => ({ ...current, [type]: clamp((current[type] ?? 0) + delta, 0, type === "minor" ? 4 : 2) }));
  await syncAllOverlays();
  await refreshSelection();
}
...
function bindUiEvents() {
  ...
  document.querySelectorAll("[data-step]").forEach((button) => button.addEventListener("click", async () => { await changeStep(button.dataset.step, Number(button.dataset.delta)); }));
...
OBR.onReady(async () => {
  ...
  setInterval(() => {
    watchSelection().catch((e) => console.error("Selection watch failed", e));
  }, 300);
});
